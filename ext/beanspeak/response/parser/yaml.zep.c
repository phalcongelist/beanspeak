...
void zep_Beanspeak_Response_Parser_Yaml_yamlParse(int ht, zval *return_value, zval **return_value_ptr, zval *this_ptr, int return_value_used, zval *mode_param_ext, zval *data_param_ext TSRMLS_DC) {
	...
	if ((zephir_function_exists_ex(SS("yaml_parse") TSRMLS_CC) == SUCCESS)) {
		ZEPHIR_CALL_FUNCTION(&response, "yaml_parse", NULL, 23, data);
		zephir_check_call_status();
	...
	ZEPHIR_SINIT_VAR(_7);
	ZVAL_STRING(&_7, "#[\r\n]+#", 0);
	ZEPHIR_CALL_FUNCTION(&lines, "preg_split", NULL, 24, &_7, _6);
	zephir_check_call_status();
	...
	if (_8) {
		...
		ZEPHIR_CALL_FUNCTION(NULL, "array_shift", NULL, 25, lines);
		ZEPHIR_UNREF(lines);
		zephir_check_call_status();
	}
	...
	if (_10) {
		ZEPHIR_CALL_FUNCTION(&_11$$8, "print_r", NULL, 26, lines, ZEPHIR_GLOBAL(global_true));
		zephir_check_call_status();
		ZEPHIR_INIT_VAR(_12$$8);
		ZEPHIR_CONCAT_SV(_12$$8, "YAML parse error. Raw data: ", _11$$8);
		ZEPHIR_SINIT_VAR(_13$$8);
		ZVAL_LONG(&_13$$8, 512);
		ZEPHIR_CALL_FUNCTION(NULL, "trigger_error", &_14, 27, _12$$8, &_13$$8);
		zephir_check_call_status();
		array_init(return_value);
		RETURN_MM();
	}
	...
	for (
	  ...
	  ; zephir_hash_move_forward_ex(_16, &_15)
		...
		if (!(zephir_array_isset_long(value, 1))) {
			...
			ZEPHIR_CALL_FUNCTION(NULL, "trigger_error", &_14, 27, _18$$11, &_19$$11);
			zephir_check_call_status();
		} else {
