...
PHP_METHOD(Beanspeak_Connection, write) {
	...
	while (1) {
		...
		if (_2$$3) {
			...
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(NULL, _3$$4, "__construct", &_7, 5, _5$$4);
			zephir_check_call_status();
...
PHP_METHOD(Beanspeak_Connection, read) {
	...
	ZEPHIR_SINIT_VAR(_1);
	ZVAL_LONG(&_1, length);
	ZEPHIR_CALL_FUNCTION(&data, "stream_get_line", NULL, 6, socket, &_1);
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&meta, "stream_get_meta_data", NULL, 7, socket);
	zephir_check_call_status();
...
PHP_METHOD(Beanspeak_Connection, getLine) {
	...
	do {
		...
		if (length) {
			...
			ZEPHIR_CALL_FUNCTION(&data, "fgets", &_2, 8, socket, &_1$$5);
			zephir_check_call_status();
		} else {
			ZEPHIR_CALL_FUNCTION(&data, "fgets", &_2, 8, socket);
			zephir_check_call_status();
		}
		ZEPHIR_CALL_FUNCTION(&meta, "stream_get_meta_data", &_3, 7, socket);
		zephir_check_call_status();
