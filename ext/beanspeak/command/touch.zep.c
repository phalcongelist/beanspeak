...
PHP_METHOD(Beanspeak_Command_Touch, __construct) {
	...
	if (_0) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(_5$$5, "beanspeak/command/touch.zep", 53 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Beanspeak_Command_Touch, parseResponse) {
	...
	if (zephir_start_with_str(line, SL("NOT_FOUND"))) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(_1$$4, "beanspeak/command/touch.zep", 84 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_INIT_VAR(_5);
	...
	zephir_check_call_status();
	zephir_throw_exception_debug(_5, "beanspeak/command/touch.zep", 87 TSRMLS_CC);
	ZEPHIR_MM_RESTORE();
	return;
