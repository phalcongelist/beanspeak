...
PHP_METHOD(Beanspeak_Command_KickJob, __construct) {
	...
	if (_0) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(_5$$5, "beanspeak/command/kickjob.zep", 54 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Beanspeak_Command_KickJob, parseResponse) {
	...
	if (zephir_start_with_str(line, SL("NOT_FOUND"))) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(_1$$4, "beanspeak/command/kickjob.zep", 85 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_INIT_VAR(_5);
	...
	zephir_check_call_status();
	zephir_throw_exception_debug(_5, "beanspeak/command/kickjob.zep", 88 TSRMLS_CC);
	ZEPHIR_MM_RESTORE();
	return;
